...
def build_engine(onnx_file_path, tensorrt_path="grounded_dino_swinT.trt"):
    ...
    # Implement and set up an appropriate calibrator for INT8 calibration

        # config.set_flag(trt.BuilderFlag.FP32)  # Example for FP16 precision
        ...
        if not network.has_implicit_batch_dimension:
            ...
            # Image input
            image_input_shape = (3, 800, 1200)  # Based on the script
            min_image_shape = (1, *image_input_shape)
            opt_image_shape = (1, *image_input_shape)
            max_image_shape = (1, *image_input_shape)
            profile.set_shape(network.get_input(0).name, min_image_shape, opt_image_shape, max_image_shape)

            # Text-related inputs (assuming a maximum sequence length, e.g., 256)
            max_seq_len = 6 # 256  # Adjust based on your actual maximum sequence length
            for i in range(1, network.num_inputs - 1):  # Exclude the last input (text_token_mask)
                text_input_shape = (max_seq_len,)
                min_text_shape = (1, *text_input_shape)
                opt_text_shape = (1, *text_input_shape)
                max_text_shape = (1, *text_input_shape)
                profile.set_shape(network.get_input(i).name, min_text_shape, opt_text_shape, max_text_shape)

            # Text token mask input
            text_token_mask_shape = (max_seq_len, max_seq_len)
            min_text_token_mask_shape = (1, *text_token_mask_shape)
            opt_text_token_mask_shape = (1, *text_token_mask_shape)
            max_text_token_mask_shape = (1, *text_token_mask_shape)
            profile.set_shape(network.get_input(network.num_inputs - 1).name, 
                              min_text_token_mask_shape, 
                              opt_text_token_mask_shape, 
                              max_text_token_mask_shape)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--onnx_path", "-op", type=str, default="../grounded.onnx", required=True, help="path to onnx file to convert")
    ...
    onnx_file_path = onnx_path # 'grounded_quantized.onnx'
    trt_engine = build_engine(onnx_file_path, tensorrt_path)
    ...
    # Now if we got this far we created a TensorRT engine successfully
        
    # Testing the engine inference below
    ...
    with trt_engine.create_execution_context() as context:
        ...
        inputs = prepare_text_inputs(
            tokenizer, image, captions, specical_tokens, max_text_len=config_args.max_text_len)

        # Copy data to buffers
        np.copyto(h_buffers[0].host, inputs["img"])  # Image data
        ...
        iterations = 1  # You can adjust the number of iterations
